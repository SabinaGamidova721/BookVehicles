...
$sql = "SELECT o.id, o.vehicle_id, o.userprofile_id, o.startdate, o.enddate, o.total_price, o.startpoint, o.endpoint, o.status_id, t.title AS type_title, s.title AS status_title, p_start.location AS start_point, p_end.location AS end_point
        FROM orders o
        JOIN vehicles v ON o.vehicle_id = v.id
        JOIN types t ON v.type_id = t.id
        JOIN statuses s ON o.status_id = s.id
        JOIN points p_start  ON o.startpoint = p_start.id
        JOIN points p_end ON o.endpoint = p_end.id
        WHERE o.userprofile_id = :user_id";

// // Добавляем условие фильтрации по типу
if ($type_filter) {
    $sql .= " AND t.title = :type_title";
}

// Добавляем условие фильтрации по статусу
if ($status_filter) {
    $sql .= " AND s.title = :status_title";
}
...
if ($type_filter) {
    $stmt->bindParam(':type_title', $type_filter, PDO::PARAM_STR);
}

if ($status_filter) {
    $stmt->bindParam(':status_title', $status_filter, PDO::PARAM_STR);
}
...
<head>
    <meta charset="utf-8">
    <title>GoodTrip | My orders</title>
                    ...
                    <div class="position-sticky top-0">
                        <div class="text-center pt-5">
                            ...
                            <div id="account-menu" class="list-group list-group-flush collapse d-md-block">
                                ...
                                <a href="all-user-orders.php" class="list-group-item list-group-item-action d-flex align-items-center active">
                                    All my orders
                                </a>
                ...
                <div class="col-md-8 offset-lg-1 pb-5 mb-lg-2 pt-md-5 mt-n3 mt-md-0">
                    <div class="ps-md-3 ps-lg-0 mt-md-2 pt-md-4 pb-md-2">
                        <div class="d-sm-flex align-items-center justify-content-between pt-xl-1 mb-3 pb-3">
                            <h1 class="h2 mb-sm-0">My orders</h1>
                            <div class="d-sm-flex align-items-center justify-content-between">
                                <form method="GET" action="">
                                    <div class="d-sm-flex align-items-center justify-content-between">
                                    <select name="type" class="form-select me-3" style="max-width: 15rem;" onchange="this.form.submit()">
                                    ...
                                    <select name="status" class="form-select" style="max-width: 15rem;" onchange="this.form.submit()">
                        ...
                        <div class="card border-0 shadow-sm overflow-hidden mb-4">
                            <div class="row g-0">
                                <div class="col-sm-4 d-flex align-items-center justify-content-center">
                                    <img src="img/<?php echo htmlspecialchars($order['type_title']); ?>.png" class="img-fluid" style="max-width: 200px;" alt="Cover image">
                                </div>
                                <div class="col-sm-8">
                                    <div class="card-body">
                                        <div class="fs-sm text-muted mb-1"><?php echo htmlspecialchars($order['status_title']); ?></div>
                                        <h2 class="h4 pb-1 mb-2">
                                            <a href="finish_trip.php"><?php echo htmlspecialchars($order['start_point']); ?> &#8594; <?php echo htmlspecialchars($order['end_point']); ?></a>
                                        </h2>
                                        <p class="mb-4 mb-lg-5"> Start date: <?php echo htmlspecialchars($order['startdate']); ?><br>
                                        <?php if ($order['total_price'] === NULL && $order['enddate'] === NULL): ?>
                                            <a href="finish_trip.php?order_id=<?php echo $order['id']; ?>" class="btn btn-outline-primary px-3 px-lg-4 me-3 mt-3">
                                                Finish trip
                                            </a>
                                        <?php elseif ($order['total_price'] !== NULL && $order['enddate'] !== NULL): ?>
                                            End date: <?php echo htmlspecialchars($order['enddate']); ?></p>
                                            <p class="mb-4 mb-lg-5">Total Price: $<?php echo htmlspecialchars($order['total_price']); ?></p>
                                            <a href="order.php?order_id=<?php echo $order['id']; ?>" class="btn btn-outline-primary px-3 px-lg-4 me-3 mt-3">
                                                Leave Comment
                                            </a>
                                        <?php endif; ?>
                                    </div>
