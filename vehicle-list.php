...
if ($type) {
    $query .= " AND types.title = :type";
    $params[':type'] = $type;
}

if ($year) {
    $query .= " AND manufacturer_year = :year";
    $params[':year'] = $year;
}

if ($max_price) {
    $query .= " AND minute_price <= :max_price";
...
<!--                            </li>-->
        ...
        <div class="container overflow-hidden pt-2 pt-md-4 pt-lg-5">
            <h1 class="mt-5">Vehicle List</h1>

            <form method="get" class="mb-5">
                <div class="form-row">
                    <div class="col">
                        <select name="type" class="form-control pb-0">
                            <option value="">All Types</option>
                            <option value="bike" <?= $type == 'bike' ? 'selected' : '' ?>>Bike</option>
                            <option value="scooter" <?= $type == 'scooter' ? 'selected' : '' ?>>Scooter</option>
                            <option value="hoverboard" <?= $type == 'hoverboard' ? 'selected' : '' ?>>Hoverboard</option>
                        </select>
                    </div>
                    <div class="col">
                        <input type="number" name="year" class="form-control" placeholder="Manufacture Year" value="<?= htmlspecialchars($year) ?>">
                    </div>
                    <div class="col">
                        <input type="number" name="max_price" class="form-control" placeholder="Max Minute Price" value="<?= htmlspecialchars($max_price) ?>" step="0.01">
                    </div>
                    <div class="col">
                ...
                <?php foreach ($vehicles as $vehicle): ?>
                    <div class="col-md-4">
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <h5 class="card-title"><?php echo htmlspecialchars($vehicle['id']); ?>: <?= htmlspecialchars($vehicle['brand_title']) ?></h5>
                                ...
                                <a href="vehicle-info.php?id=<?= $vehicle['id'] ?>" class="btn btn-primary">View Details</a>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
